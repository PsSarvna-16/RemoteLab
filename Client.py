...
connectServer(soc.ip,soc.port)

update_b = Button(window, text ="Update Changes", command = lambda:sendData(str(float(slider.get())),soc.con))
update_b.pack(pady =10 )

exit_b = Button(window, text ="Exit", command = lambda:sendData("EXIT",soc.con))
exit_b.pack(padx =20 , side = LEFT)

term_b = Button(window, text ="Terminate", command = lambda:sendData("TERMINATE",soc.con))
